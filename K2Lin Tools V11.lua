...
local Y_level = 3				------ Vertical move per step. 										Default = 1(=<0 to work)
...
local x_composite_enabled = true		------ WIP horizontal recoil suppression								Default = true
...
local ReleseDate = "2025/3/1"			------ Type whatever you want										Default = A date
...
local mode_rapid_2_default_delay = 20
local mode_rapid_2_default_Y_level = 2
local mode_rapid_3_name = "Pistol-Slow"
local mode_rapid_3_default_delay = 20
local mode_rapid_3_default_Y_level = 4
local mode_rapid_4_name = "Pistol-Fast"
local mode_rapid_4_default_delay = 20
local mode_rapid_4_default_Y_level = 6
local mode_rapid_5_name = "OTG-41-Slow"
local mode_rapid_5_default_delay = 20
local mode_rapid_5_default_Y_level = 9
local mode_rapid_6_name = "OTG-41-Fast"
local mode_rapid_6_default_delay = 20
local mode_rapid_6_default_Y_level = 30
------------------------------------------------		Main Module			--------------------------------------------------
...
function OnEvent(event, arg)
    --OutputLogMessage("Key: ".. arg.. "\n")
 	if ( event == "PROFILE_ACTIVATED" ) then
		...
		Sleep(250)
		ClearLog()
		OutputLogMessage("[			#===K2LINTECH v11 ===||[_G-series Lua API _]||===#			]\n");
	...
	if(event == "MOUSE_BUTTON_PRESSED" and Recoil) then
		firing (0,Y_level,Delay) --Calls function
		log()
	elseif(event == "MOUSE_BUTTON_PRESSED" and arg == rapid_button and Automatic) then
		firing (0,rapid_Y_level,rapid_Delay) --Calls Rapid function
		log()
end
end---end onevent()
...
function log()
	...
	if(debug == true)then
		...
		OutputLogMessage(" Click Per Secound \n")
	...
	OutputLogMessage("\n")
	OutputLogMessage("-----Clantag-K2LINGROUP BETA V11 	ReleseDate:"..ReleseDate.."-----\n")
	end
end
...
function firing(a,b,rsleep)
		...
		if IsMouseButtonPressed(3) then
			if(IsMouseButtonPressed(1)) then
				repeat
					...
					if (Recoil == true) then
					if(x_composite_enabled) then
						x_rec = math.random(-3,1)
						MoveMouseRelative(x_rec,0)
					end
					if(autotag) then
						PressKey(tagbutton)
    ...
    elseif (Master and Automatic) then
		if IsMouseButtonPressed(3) then
			if IsMouseButtonPressed(rapid_button) then
				repeat
					Recoil_Step1 = math.random(1,2)
					if (Automatic) then
					PressKey(autofirekey)
					if(autotag) then
					PressKey(tagbutton)
					Sleep(math.random(5,25))
					ReleaseKey(tagbutton)
					Sleep(math.random(5,25))
						...
						time = GetRunningTime()
							...
							elseif(Recoil_Step == 3)then
								...
								Recoil_Step = math.random(4,7)
							elseif(Recoil_Step == 4)then
								MoveMouseRelative(a,b+3)
								Sleep(Delay-1)
								Recoil_Step = math.random(5,7)
							elseif(Recoil_Step == 5)then
								MoveMouseRelative(a,b+5)
								Sleep(Delay)
								Recoil_Step = math.random(1,4)
							elseif(Recoil_Step == 6)then
								MoveMouseRelative(a,b+2)
					...
					Sleep(rapid_delay)
					ReleaseKey(autofirekey)
					Sleep(rapid_firerate)
						ClearLog()
						OutputLogMessage("Function Cycle runs in : ")
						OutputLogMessage(timer+rapid_firerate)
						OutputLogMessage(" millisecond(s)\n")
					end
				until (not IsMouseButtonPressed(rapid_button) or not IsMouseButtonPressed(3))
				firstinterval = 0
				timerlast = timer
				end
			end
